...
import { resolveCacheRoot, resolveSessionRoot } from "./paths.js";
...
export async function syncCache(options = {}) {
    return withLock(options, async (ctx) => {
        let manifest = await readManifest(ctx);
        if (invalidManifest(manifest, ctx))
            manifest = freshManifest(ctx);
        ...
        for (const sourcePath of files) {
            ...
            if (previous && previous.mtime === st.mtimeMs && previous.size === st.size && await sourceCacheComplete(previous))
                continue;
...
export async function doctorCache(options = {}) {
    const ctx = context(options);
    ...
    if (manifestInvalid)
        issues.push("manifest metadata does not match current cache configuration");
    for (const [sourcePath, source] of Object.entries(manifestInvalid ? {} : manifest.sources)) {
        if (!(await exists(sourcePath)))
            issues.push(`missing source: ${sourcePath}`);
...
function invalidManifest(manifest, ctx) {
...
function validManifestOrFresh(manifest, ctx) {
    return invalidManifest(manifest, ctx) ? freshManifest(ctx) : manifest;
}
function isManifestShape(manifest) {
    if (!manifest || typeof manifest !== "object" || typeof manifest.sources !== "object" || manifest.sources === null || Array.isArray(manifest.sources))
        return false;
    return Object.values(manifest.sources).every(isCacheSourceEntry);
}
function isCacheSourceEntry(source) {
...
async function readManifest(ctx) {
    try {
        return JSON.parse(await readFile(join(ctx.cacheRoot, "manifest.json"), "utf8"));
    }
    catch {
        return freshManifest(ctx);
    }
}
async function writeManifest(ctx, manifest) { await atomicWrite(join(ctx.cacheRoot, "manifest.json"), JSON.stringify(manifest, null, 2)); }
async function ensureLayout(cacheRoot) { await mkdir(join(cacheRoot, "records"), { recursive: true, mode: 0o700 }); await mkdir(join(cacheRoot, "sessions"), { recursive: true, mode: 0o700 }); await writeFile(join(cacheRoot, CACHE_SENTINEL), "pi-fzf cache\n", { mode: 0o600 }); }
async function atomicWrite(path, content) { await mkdir(dirname(path), { recursive: true, mode: 0o700 }); const tmp = `${path}.tmp-${process.pid}-${Date.now()}`; await writeFile(tmp, content, { mode: 0o600 }); await rename(tmp, path); }
async function removeSourceShards(source) { await rm(source.paths.records, { force: true }); await rm(source.paths.session, { force: true }); }
async function sourceCacheComplete(source) { return (await sourceCacheIssue(source)) === undefined; }
async function sourceCacheIssue(source) {
    if (!(await exists(source.paths.records)))
        return `missing records shard: ${source.paths.records}`;
    const recordsIssue = await validateRecordsShard(source.paths.records, source.recordCount);
    if (recordsIssue)
        return recordsIssue;
    if (!(await exists(source.paths.session)))
        return `missing session metadata: ${source.paths.session}`;
    try {
        const metadata = JSON.parse(await readFile(source.paths.session, "utf8"));
        if (!metadata || typeof metadata !== "object")
            return `corrupt session metadata: ${source.paths.session}`;
    }
    catch {
        return `corrupt session metadata: ${source.paths.session}`;
    }
...
async function validateRecordsShard(path, expectedCount) {
    let count = 0;
    try {
        ...
        return `corrupt records shard: ${path}`;
    }
...
async function validateCacheRootForDestructiveOperation(ctx) {
    ...
    if (forbidden.has(cacheRoot))
        throw new Error(`refusing to rebuild unsafe cache root: ${ctx.cacheRoot}`);
    if (pathsOverlap(cacheRoot, sessionRoot))
        throw new Error(`refusing to rebuild cache root overlapping session root: ${ctx.cacheRoot}`);
    ...
    if (await exists(cacheRoot)) {
        ...
        if (realCacheRoot === realParent || forbidden.has(realCacheRoot))
            throw new Error(`refusing to rebuild unsafe cache root: ${ctx.cacheRoot}`);
...
function isSameOrChildRelativePath(value) {
    return value === "" || (!value.startsWith("..") && !parse(value).root);
}
async function removeCacheOwnedContents(cacheRoot) {
...
async function listFiles(dir) {
    try {
        return (await readdir(dir, { withFileTypes: true })).filter((e) => e.isFile()).map((e) => join(dir, e.name));
    }
    catch {
        return [];
    }
