...
export async function parseSessionFile(sessionPath) {
    ...
    for await (const line of rl) {
        ...
        if (!cwd && metadata.cwd)
            cwd = metadata.cwd;
        if (!sessionName && metadata.sessionName)
        ...
        if (isNoisyTopLevelEntry(entry))
            continue;
        const message = messageEnvelope(entry);
        if (!message || !TEXT_ROLES.has(message.role) || NOISY_ROLES.has(message.role))
            continue;
    ...
    const parsed = { sessionId, sourcePath, records };
    if (cwd !== undefined)
        parsed.cwd = cwd;
...
function baseRecord(entry, role, text, sequence, cwd, sessionName) {
    const record = { role, text, sequence };
...
function sessionMetadata(entry) {
    ...
    const result = {};
    const sessionId = stringValue(metadata.sessionId) ?? stringValue(metadata.session_id) ?? (isSessionHeader ? stringValue(metadata.id) : undefined) ?? stringValue(nested.id);
    const cwd = stringValue(metadata.cwd) ?? stringValue(nested.cwd);
    const sessionName = stringValue(metadata.name) ?? stringValue(metadata.sessionName) ?? stringValue(metadata.session_name) ?? stringValue(nested.name);
...
function messageEnvelope(entry) {
...
function safeSessionIdFromPath(sourcePath) {
    return basename(sourcePath, ".jsonl") || sourceKeyFragment(sourcePath);
}
function sourceKeyFragment(value) {
    let hash = 0;
...
function summaryText(entry) {
    ...
    if (compaction)
        return { role: "compaction", text: compaction };
    ...
    if (branch)
        return { role: "branch_summary", text: branch };
    return undefined;
}
function isNoisyTopLevelEntry(entry) {
    const type = stringValue(entry.type);
    return type !== undefined && NOISY_TYPES.has(type);
}
function extractTextBlocks(content) {
    if (typeof content === "string")
        return noisyText(content) ? [] : [content];
    ...
    for (const block of content) {
        ...
        if (!type || type !== "text" || NOISY_TYPES.has(type))
            continue;
        const text = stringValue(block.text);
        if (text && !noisyText(text))
            texts.push(text);
    }
    return texts;
}
function noisyText(text) {
